...
canonicalization_network = ESCNNEquivariantNetwork(
    ...
    3,              # kernel_size
    "rotation",     # group_type
    8,              # num_rotations (C8 group for 45-degree intervals)
    3               # num_layers
).to(device)

# 2. Define Hyperparameters according to the PRLC Paper
canonicalization_hyperparams = OmegaConf.create({
    "group_type": "rotation",
    "num_rotations": 8,
    "alpha": 1.0, 
    "beta": 100.0,              # Prior weight beta=100 as per PRLC paper 
...
# 4. Load the ResNet50 backbone (Pre-trained on ImageNet-1K)
...
# 6. Dataset setup (CIFAR10 used in the FOCAL/PRLC benchmarks)
...
# 7. Training Loop: Joint Training Protocol 
prediction_network.train()
canonicalization_network.train()

for epoch in range(100): # PRLC is typically trained until convergence 
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        # PRLC Total Loss: Task Cross-Entropy + Beta * Prior Loss
